...
def prepared_list(data):
    ...
    if crc_recv != crc:
        print("Corrupted message with Message-ID {}, some packages were lost\n".format(data[0]))
    else:
        print("\nMessage-ID: {}".format(data[0]))
        print("From: {}".format(data[1]))
        print("To: {}".format(data[2]))
        print("Message: {}".format(msg_recv))
...
def add_user(user, passwd, type_of_user_add, group):
    mas = [user, passwd, type_of_user_add, group]
    code = making_code(mas, 4)
...
def add_user_to_group(user, group):
    ...
    with open("base_of_users.bin", 'rb') as input_users:
        for line in input_users:
            data = line.decode().split('\t')
            if data[0] == user:
                if data[3][len(data[3]) - 1] != '\n':
                    group_user = data[3]
                else:
                    group_user = data[3][0:(len(data[3]) - 1)]
                found_user = True
                break
    found_group = False
    with open("base_of_groups.bin", 'rb') as input_groups:
    ...
    with open("base_of_messages.bin", 'rb') as input_messages:
        for line in input_messages:
            ...
            if data[2] == user and checked == '0':
                ...
                msg_crc = data[3].split("c:::")
                msg_recv = msg_crc[0]
                print("Message-ID: {} From: {} To: {} Message: {}".format(data[0], data[1], data[2], msg_recv))
    return have_msg
    ...
    with open("base_of_messages.bin", 'rb+') as input_messages:
        for line in input_messages:
            ...
            if data[6][length - 1] != '\n':
                ...
                checked = data[6][0:(length - 1)]
            if ((data[1] == user and data[2] == to) or (data[1] == to and data[2] == user)) and data[0] == id_msg \
                    and checked == '1':
                have_msg = True
                prepared_list(data)
                break
    if not have_msg:
        print("No message with Message-ID {}\n".format(id_msg))
            ...
            while True:
                ...
                if command.lower() == "exit":
                    ...
                    if check_groups(name_of_group):
                        ...
                        print("Sorry, but group with group_name {} already exists in the group's database".format(
                            name_of_group))
                elif (command.lower() == "add user to group" or command.lower() == "add to group") \
                        and type_of_user == 'a':
                    while True:
                        print()
                        if print_users(username):
                            ...
                            break
                elif command.lower() == "write message" or command.lower() == "view history of messages" \
                        or command.lower() == "write" or command.lower() == "view":
                    print()
